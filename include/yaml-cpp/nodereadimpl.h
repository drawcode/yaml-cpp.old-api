...
// (the goal is to call ConvertScalar if we can, and fall back to operator >> if
// not)
...
// Note: this doesn't work on gcc 3.2, but does on gcc 3.4 and above. I'm not
// sure about 3.3.
...
inline bool Node::Read(T& value) const {
  ...
  return read_impl<sizeof(fallback::flag(), Convert(std::string(), value),
