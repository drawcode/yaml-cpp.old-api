...
struct TagMismatch : public std::exception {
  TagMismatch(const std::string& actualTag, const std::string& expectedTag) {
    ...
    output << "Tag has value \"" << actualTag << "\" but \"" << expectedTag
           << "\" was expected";
    what_ = output.str();
  }
...
bool Infinity() {
  ...
  for (unsigned i = 0; i < doc.size(); i++)
    if (doc[i].to<double>() !=
        (i < 6 ? +1 : -1) * std::numeric_limits<double>::infinity())
      return false;
  for (unsigned i = 0; i < doc.size(); i++)
    if (doc[i].to<long double>() !=
        (i < 6 ? +1 : -1) * std::numeric_limits<long double>::infinity())
      return false;
  for (unsigned i = 0; i < doc.size(); i++)
    if (doc[i].to<float>() !=
        (i < 6 ? +1 : -1) * std::numeric_limits<float>::infinity())
      return false;
  return true;
}
...
bool RunParserTests() {
  ...
  RunParserTest(&Parser::FlowMapWithOmittedKey, "flow map with omitted key",
                passed, total);
  RunParserTest(&Parser::FlowMapWithOmittedValue, "flow map with omitted value",
                passed, total);
  RunParserTest(&Parser::FlowMapWithSoloEntry, "flow map with solo entry",
                passed, total);
  RunParserTest(&Parser::FlowMapEndingWithSoloEntry,
                "flow map ending with solo entry", passed, total);
  ...
  RunParserTest(&Parser::MultipleDocsWithSomeExplicitIndicators,
                "multiple docs with some explicit indicators", passed, total);
  RunParserTest(&Parser::BlockKeyWithNullValue, "block key with null value",
                passed, total);
  ...
  RunParserTest(&Parser::DefaultPlainScalarTag, "default plain scalar tag",
                passed, total);
  RunParserTest(&Parser::DefaultSingleQuotedScalarTag,
                "default single-quoted scalar tag", passed, total);
  RunParserTest(&Parser::ExplicitNonSpecificPlainScalarTag,
                "explicit, non-specific plain scalar tag", passed, total);
  ...
  RunParserTest(&Parser::StandardShortcutTag, "standard shortcut tag", passed,
                total);
  RunParserTest(&Parser::VerbatimURITag, "verbatim URI tag", passed, total);
  RunParserTest(&Parser::DefaultPlainScalarTag, "default plain scalar tag",
                passed, total);
  RunParserTest(&Parser::DefaultSequenceTag, "default sequence tag", passed,
                total);
  RunParserTest(&Parser::ExplicitNonSpecificSequenceTag,
                "explicit, non-specific sequence tag", passed, total);
  ...
  RunParserTest(&Parser::DereferenceScalar, "dereference scalar", passed,
                total);
  RunParserTest(&Parser::DereferenceKeyScalar, "dereference key scalar", passed,
                total);
  RunParserTest(&Parser::DereferenceValueScalar, "dereference value scalar",
                passed, total);
  RunParserTest(&Parser::DereferenceKeySeq, "dereference key seq", passed,
                total);
  RunParserTest(&Parser::DereferenceValueSeq, "dereference value seq", passed,
                total);
