...
TEST MappingBetweenSequences() {
  ...
  YAML_ASSERT(doc[Pair("Detroit Tigers", "Chicago cubs")][0]
                  .to<std::string>() == "2001-07-23");
  YAML_ASSERT(doc[Pair("New York Yankees", "Atlanta Braves")].size() == 3);
  YAML_ASSERT(doc[Pair("New York Yankees", "Atlanta Braves")][0]
                  .to<std::string>() == "2001-07-02");
  YAML_ASSERT(doc[Pair("New York Yankees", "Atlanta Braves")][1]
                  .to<std::string>() == "2001-08-12");
  YAML_ASSERT(doc[Pair("New York Yankees", "Atlanta Braves")][2]
                  .to<std::string>() == "2001-08-14");
  return true;
}
...
TEST IndentationDeterminesScope() {
  ...
  YAML_ASSERT(doc["accomplishment"].to<std::string>() ==
              "Mark set a major league home run record in 1998.\n");
  YAML_ASSERT(doc["stats"].to<std::string>() ==
...
TEST MultiLineFlowScalars() {
  PARSE(doc, ex2_18);
  YAML_ASSERT(doc.size() == 2);
  YAML_ASSERT(doc["plain"].to<std::string>() ==
              "This unquoted scalar spans many lines.");
  YAML_ASSERT(doc["quoted"].to<std::string>() ==
...
TEST Invoice() {
  ...
  YAML_ASSERT(doc["bill-to"]["address"]["lines"].to<std::string>() ==
              "458 Walkman Dr.\nSuite #292\n");
  YAML_ASSERT(doc["bill-to"]["address"]["city"].to<std::string>() ==
              "Royal Oak");
  ...
  YAML_ASSERT(doc["ship-to"]["address"]["lines"].to<std::string>() ==
              "458 Walkman Dr.\nSuite #292\n");
  YAML_ASSERT(doc["ship-to"]["address"]["city"].to<std::string>() ==
              "Royal Oak");
  ...
  YAML_ASSERT(doc["product"][0]["description"].to<std::string>() ==
              "Basketball");
  ...
  YAML_ASSERT(doc["product"][1]["description"].to<std::string>() ==
              "Super Hoop");
...
TEST LogFile() {
  ...
  YAML_ASSERT(doc["Warning"].to<std::string>() ==
  ...
  YAML_ASSERT(doc["Stack"][0]["code"].to<std::string>() ==
              "x = MoreObject(\"345\\n\")\n");
...
TEST NodePropertyIndicators() {
  PARSE(doc, ex5_6);
  YAML_ASSERT(doc.size() == 2);
  YAML_ASSERT(doc["anchored"].to<std::string>() ==
...
TEST IndentationSpaces() {
  ...
  YAML_ASSERT(doc["Not indented"]["By one space"].to<std::string>() ==
              "By four\n  spaces\n");
  YAML_ASSERT(doc["Not indented"]["Flow style"].size() == 3);
  YAML_ASSERT(doc["Not indented"]["Flow style"][0].to<std::string>() ==
              "By two");
  YAML_ASSERT(doc["Not indented"]["Flow style"][1].to<std::string>() ==
              "Also by two");
  YAML_ASSERT(doc["Not indented"]["Flow style"][2].to<std::string>() ==
              "Still by two");
  return true;
}
...
TEST InvalidTagShorthands() {
  ...
  PARSE(doc, ex6_27b);  // TODO: should we reject this one (since !h! is not
                        // declared)?
...
TEST BlockScalarNodes() {
  PARSE(doc, ex8_21);
  YAML_ASSERT(doc.size() == 2);
  YAML_ASSERT(doc["literal"].to<std::string>() == "value");  // Note: I believe
                                                             // this is a bug in
                                                             // the YAML spec -
                                                             // it should be
                                                             // "value\n"
